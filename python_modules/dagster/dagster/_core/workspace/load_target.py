...
from dagster._core.workspace.load import (
    location_origin_from_autodefs_module_name,
...
def get_origins_from_toml(
    ...
    with open(path, "rb") as f:
        ...
        if dg_block:
            if autodefs_module := dg_block.get("autodefs_module"):
                return AutodefsModuleTarget(
                    autodefs_module_name=autodefs_module,
                    working_directory=os.getcwd(),
...
@record(kw_only=False)
class AutodefsModuleTarget(WorkspaceLoadTarget):
    autodefs_module_name: str
    working_directory: Optional[str]
    location_name: Optional[str]
